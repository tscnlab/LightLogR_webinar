...
`gg_states()` augments an existing plot by adding background rectangles that mark state intervals. When multiple states are present, map them to distinct fills (or colors) to improve readability.
...
dataset_1day |> 
  ...
  gg_states(State.colname = state, aes_fill = state) +
